...
type Kenshu = 'umaren' | 'umatan' | 'sanrenpuku' | 'sanrentan';
...
const KENSHU_OPTIONS: { value: Kenshu; label: string }[] = [
...
const KAIKATA_BY_KENSHU: Record<Kenshu, { value: Kaikata; label: string }[]> = {
  umaren: [
...
function getGroupConfigs(kenshu: Kenshu, kaikata: Kaikata): GroupConfig[] {
  if (kaikata === 'normal') {
    if (kenshu === 'umaren')
      return [
        { label: '1頭目', max: 1 },
  ...
  if (kaikata === 'nagashi_1chaku')
    return [
      { label: '軸（1着）', max: 1 },
      { label: '相手', max: Infinity },
    ];
  if (kaikata === 'nagashi_2chaku')
    return [
      { label: '軸（2着）', max: 1 },
      { label: '相手', max: Infinity },
    ];
  if (kaikata === 'nagashi_3chaku')
    return [
      { label: '軸（3着）', max: 1 },
      { label: '相手', max: Infinity },
    ];
  if (kaikata === 'nagashi_12chaku')
    return [
      { label: '軸（1・2着）', max: 2 },
      { label: '相手（3着）', max: Infinity },
    ];
  if (kaikata === 'nagashi_23chaku')
    return [
      { label: '軸（2・3着）', max: 2 },
      { label: '相手（1着）', max: Infinity },
    ];
  if (kaikata === 'nagashi_jiku1')
    return [
      { label: '軸', max: 1 },
      { label: '相手', max: Infinity },
    ];
  if (kaikata === 'nagashi_jiku2')
    return [
      { label: '軸', max: 2 },
      { label: '相手', max: Infinity },
    ];
...
function calculatePoints(bet: Bet): number {
...
function umarenPoints(kaikata: Kaikata, g1: number[], g2: number[]): number {
  switch (kaikata) {
    ...
    case 'formation': {
      ...
      for (const a of g1)
        for (const b of g2) if (a !== b) set.add([a, b].sort((x, y) => x - y).join(','));
      return set.size;
    }
    case 'nagashi':
...
function betToText(bet: Bet): string {
  ...
  const kaikataLabel =
    KAIKATA_BY_KENSHU[bet.kenshu]?.find((k) => k.value === bet.kaikata)?.label || '';
...
function generateShareText(race: Race): string {
...
function createSampleRace(): Race {
...
function YosoTab({ race, onChange }: YosoTabProps) {
...
function MarkDropdown({ value, onChange }: MarkDropdownProps) {
...
function HorseRow({ horse, onUpdate }: HorseRowProps) {
...
function KaimeTab({ race, onChange, onShare }: KaimeTabProps) {
...
function BetCard({ bet, onEdit, onDelete }: BetCardProps) {
  const kenshuLabel = KENSHU_OPTIONS.find((k) => k.value === bet.kenshu)?.label || '';
  const kaikataLabel =
    KAIKATA_BY_KENSHU[bet.kenshu]?.find((k) => k.value === bet.kaikata)?.label || '';
  const points = calculatePoints(bet);
...
function BetModal({ race, bet, onSave, onCancel }: BetModalProps) {
  ...
  const setters: Dispatch<SetStateAction<number[]>>[] = [setG1, setG2, setG3];
  const values: number[][] = [g1, g2, g3];
          ...
          <div key={cfg.label}>
            <div className="flex items-baseline justify-between mb-2">
              ...
              <div className="font-num text-xs" style={{ color: 'var(--muted)' }}>
                {values[i].length}頭{cfg.max !== Infinity && cfg.max > 1 && ` / 最大${cfg.max}頭`}
              </div>
            </div>
            <HorseGrid
...
function ShareModal({ race, onClose }: ShareModalProps) {
...
function RaceDetail({ race, onBack, onChange }: RaceDetailProps) {
  const [tab, setTab] = useState<'yoso' | 'kaime'>('yoso');
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="tab-bar flex">
        ...
        >
          買い目 {race.bets.length > 0 && `(${race.bets.length})`}
        </button>
      </div>
...
function HomeScreen({ races, onSelectRace, onCreateRace, onDeleteRace }: HomeScreenProps) {
...
function RaceListItem({ race, onSelect, onDelete }: RaceListItemProps) {
...
function NewRaceForm({ onCreate, onCancel }: NewRaceFormProps) {
